...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'tokacookies_ultra_2026'
bcrypt = Bcrypt(app)
...
connected_users = {} # {sid: {pseudo, mult, guild, friends: []}}
...
def get_user_friends(pseudo):
    try:
        res = supabase.table("friendships").select("*").or_(f'user1.eq."{pseudo}",user2.eq."{pseudo}"').eq("status", "accepted").execute()
        return [f['user2'] if f['user1'] == pseudo else f['user1'] for f in res.data]
    except: return []

def update_social_data(pseudo):
    try:
        friends = get_user_friends(pseudo)
...
def auth(data):
    ...
    if user and bcrypt.check_password_hash(user['password'], pwd):
        ...
        connected_users[request.sid] = {'pseudo': p, 'mult': user['multiplier'], 'guild': user.get('guild_name'), 'friends': friends}
        join_room(p)
        emit('login_ok', {'pseudo': p, 'clicks': user['clicks'], 'mult': user['multiplier'], 'guild': user.get('guild_name'), 'rank': get_rank(user['clicks'])})
        refresh_leaderboard(request.sid)
...
def click():
    ...
    if u:
        ...
        if nv % 10 == 0: refresh_leaderboard(request.sid)
        if u.get('guild'):
...
def chat(data):
    ...
    if u and data['msg'].strip():
        ...
        socketio.emit('new_chat', msg_data, broadcast=True)
